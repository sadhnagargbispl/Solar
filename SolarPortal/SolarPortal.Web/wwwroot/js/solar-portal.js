...
function showError(msg) {
    if (typeof Swal !== 'undefined') {
        Swal.fire({ icon: 'error', title: 'Error', text: msg, background: '#1a2035', color: '#f0f4ff' });
    } else { alert(msg); }
}
function showSuccess(msg) {
    if (typeof Swal !== 'undefined') {
        Swal.fire({ icon: 'success', title: 'Success', text: msg, background: '#1a2035', color: '#f0f4ff', timer: 2500, showConfirmButton: false });
    } else { alert(msg); }
}
function showConfirm(msg, cb) {
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            ...
            showCancelButton: true, confirmButtonColor: '#f5a623',
...
function uploadDocument(requestId, docType, inputId, statusId) {
    ...
    fetch('/User/SolarRequest/UploadDocument', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(d => {
            if (statusEl) {
...
function approveRequest(id) {
    showConfirm('Approve this request?', () => {
        post('/Admin/Projects/Approve', { id }, d => {
    ...
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            ...
            if (r.isConfirmed && r.value) {
                post('/Admin/Projects/Reject', { id, reason: r.value }, d => {
        ...
        if (reason) {
            post('/Admin/Projects/Reject', { id, reason }, d => {
...
function submitPayment(requestId) {
    ...
    fetch('/User/SolarRequest/AddPayment', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(d => {
